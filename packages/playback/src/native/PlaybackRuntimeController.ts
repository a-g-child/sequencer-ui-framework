      ...
      this.backend.sendCommands([
        {
          ...
          type,
          timeMs: nowMs(),
          atSample: requestedSample ?? commandSnapshot.transport.samplePosition
        } as EngineCommand
      ])
...
function nativeSchedulerDiagnostics(snapshot: RuntimeSnapshot): string {
  ...
  return [
    ...
    `firstVisit=${scheduler.firstScheduledEventVisitedSample ?? 'n/a'}`,
    `firstDispatch=${scheduler.firstScheduledEventDispatchedSample ?? 'n/a'}`,
    ...
    `stoppedSuppress=${scheduler.eventsSuppressedWhileStopped}`,
