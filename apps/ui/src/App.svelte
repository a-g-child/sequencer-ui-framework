...
{#if diagnosticsOpen}
  <RuntimePanel
      ...
      nativeAudioDriver={nativeAudioDriver}
	    ...
	    runtimeOwnerGenerationsSet={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.ownerGenerationsSet}
	    runtimeBeatEventsInserted={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.beatEventsInserted}
	    runtimeBeatEventMinSample={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.beatEventMinSample}
	    runtimeBeatEventMaxSample={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.beatEventMaxSample}
	    runtimeSampleEventsInserted={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.sampleEventsInserted}
	    runtimeNoteOnsDispatched={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.noteOnsDispatched}
	    runtimeNoteOffsDispatched={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.noteOffsDispatched}
	    runtimeEventsDiscardedOwner={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.eventsDiscardedOwner}
	    runtimeEventsDiscardedFutureOwner={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.eventsDiscardedFutureOwner}
	    runtimeEventsDroppedNotPlaying={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.eventsDroppedNotPlaying}
	    runtimeEventsDroppedCapacity={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.eventsDroppedCapacity}
	    runtimeLoopReschedules={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.loopReschedules}
	    runtimeLoopRescheduleSkippedDisabled={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.loopRescheduleSkippedDisabled}
	    runtimeLoopRescheduleSkippedOutside={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.loopRescheduleSkippedOutside}
	    runtimeEventsCleared={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.eventsCleared}
	    runtimeTransportLoopEnabled={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.transportLoopEnabled}
	    runtimeTransportLoopStartSample={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.transportLoopStartSample}
	    runtimeTransportLoopEndSample={playbackStatus.runtime?.snapshot?.diagnostics.scheduler?.transportLoopEndSample}
	    runtimeEventGraphEventsReceived={playbackStatus.runtime?.snapshot?.diagnostics.eventGraph?.eventsReceived}
	    runtimeEventGraphRouteDispatches={playbackStatus.runtime?.snapshot?.diagnostics.eventGraph?.routeDispatches}
	    runtimeEventGraphEventsEmitted={playbackStatus.runtime?.snapshot?.diagnostics.eventGraph?.eventsEmitted}
	    runtimeEventGraphEventsDroppedCapacity={playbackStatus.runtime?.snapshot?.diagnostics.eventGraph?.eventsDroppedCapacity}
	    runtimeEventGraphEventsDroppedDepth={playbackStatus.runtime?.snapshot?.diagnostics.eventGraph?.eventsDroppedDepth}
	    runtimeEventGraphEventsDroppedBudget={playbackStatus.runtime?.snapshot?.diagnostics.eventGraph?.eventsDroppedBudget}
